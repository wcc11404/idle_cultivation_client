...
### 2) 切换炼制数量（min/-10/-1/+1/+10/max）

1. 根据按钮语义增减 `selected_count`（最小始终为 1，最大受当前材料/灵气可制作上限约束）。
...
- 丹方详情中的材料需求、灵气需求：当前统一走普通数值口径（`format_display_number`），即使是计数类材料，也与顶部资源展示保持一致。
- 成丹/废丹/批次颗数等流程计数，当前仍保持整数型文案表达。
...
以下为当前炼丹房设计改造中“有必要保留”的部分，均不改变业务行为，仅做稳定性与可维护性优化：

1. 样式初始化与状态刷新分离  
   - 静态样式只初始化一次（字号、分割线、按钮底色等）。  
   - 动态状态单独刷新（按钮可点击、进度值、材料红字、计数文案）。

2. 单一渲染入口  
   - 炼丹房展示统一走 `_render_alchemy_ui(...)`，避免散落在多个函数里重复刷新，减少“改一处崩一处”风险。

3. 列表区域增量同步  
   - 丹方列表从“全量销毁重建”改为“按差异增删+复用节点+重排”。  
   - 材料需求区同样改为增量同步，仅更新变化条目。

4. 保持体验逻辑不变（强约束）  
   - 仍是每轮预扣、每轮 report 结算、失败回滚当前轮、停火统一汇总。  
   - 文案策略仍由客户端根据 `reason_code + reason_data` 组装，不回退到服务端 message 文案。

5. 当前视觉约定  
   - 炼丹房返回按钮加宽，避免小屏下可点击面积偏小。  
   - 现有配色与字号保持你确认过的版本，后续再做统一主题时再集中调整。
