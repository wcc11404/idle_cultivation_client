...
| 子系统 | 说明 | 核心文件 |
|--------|------|----------|
| 修炼系统 | 灵气积累和气血恢复 | CultivationSystem.gd |
| 境界系统 | 境界配置和突破逻辑 | RealmSystem.gd |
| 离线收益 | 离线奖励计算 | OfflineReward.gd |
| 内视UI | 修炼界面展示 | NeishiModule.gd |
...
const BREAKTHROUGH_MATERIALS = {
    "realm_breakthrough": {...},    # 大境界突破材料
    "level_breakthrough": {...}     # 小境界突破材料
}

# OfflineReward.gd
const MAX_OFFLINE_HOURS: float = 4.0     # 最大离线时间（小时）
const MIN_OFFLINE_MINUTES: float = 1.0   # 最小离线时间（分钟）
```
